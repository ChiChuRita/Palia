...
const activityCategoryEnum = z.enum(
  ACTIVITY_CATEGORY_KEYS as unknown as [
    ActivityCategory,
    ...ActivityCategory[],
  ],
);
...
export default defineAgent({
  entry: async (ctx: JobContext) => {
    ...
    console.log(
      `[mecfs-agent] locale=${locale} health=${healthSnapshot ? 'yes' : 'none'}`,
    );
    ...
    const agent = new voice.Agent({
      ...
      tools: {
        get_health_context: llm.tool({
          ...
          execute: async () =>
            JSON.stringify(formatHealthContext(healthSnapshot)),
        }),
        ...
        record_activity: llm.tool({
          ...
          parameters: z.object({
            ...
            durationMinutes: z.number().min(0).max(24 * 60).nullish(),
          }),
          execute: async (args) => {
        ...
        end_session: llm.tool({
          ...
          execute: async (args) => {
            console.log(
              `[mecfs-agent] end_session: "${args.summary}" energy=${args.energy_score}`,
            );
